...
void Laborator9::Update(float deltaTimeSeconds)
{
...

	{
		...
		// Bonus 2
		glm::vec3 cameraPos = GetSceneCamera()->transform->GetLocalPosition();
		float angle = acos(glm::dot(glm::normalize(glm::vec3(cameraPos.x, 0, cameraPos.z)), glm::vec3(0, 0, 1)));
		if (cameraPos.x < 0) {
			angle *= -1;
		}
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(1.f, 0.5f, 0.0f));
		modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	glUniform1i(glGetUniformLocation(shader->program, "two_textures"), twoTextures);

	// Bonus 1
	glUniform1f(glGetUniformLocation(shader->program, "time"), Engine::GetElapsedTime());
	glUniform1i(glGetUniformLocation(shader->program, "is_globe"), isGlobe);
